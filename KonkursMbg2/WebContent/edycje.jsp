...
<br>
<button id="dodaj" class="dodaj" onclick="document.getElementById('m01').style.display='block'" style="width:auto;">+ Dodaj nową edycję</button>
<div id="m01" class="modal">
  <form class="modal-content animate" method="post" action="/KonkursMbg/edycja">
    <div class="imgcontainer">
      <span onclick="document.getElementById('m01').style.display='none'" class="close" title="Close Modal">&times;</span>
      
    </div>
    <div class="container">
    <h2>Dodawanie nowej edycji</h2>
    <input type="hidden" id="metoda" name="metoda" value="add">
      <div>
        ...
        <div>
            <label for="active">Czy edycja jest aktywna?:</label>
		...
		<div>
            <label for="wszystkieWyniki">Czy zostały wprowadzone wszystkie wyniki?:</label>
        	<select id="wszystkieWyniki" name="wszystkieWyniki">
        		<option value=""> </option>
				<option value="1">Tak</option>
				<option value="0">Nie</option>
			</select>
...
<div id="m02" class="modal">
  <form class="modal-content animate" method="post" action="/KonkursMbg/edycja">
    ...
    <div class="container">
    <h2>Edytowanie edycji</h2>
    <input type="hidden" id="id_edycja" name="id_edycja">
...
<div id="m03" class="modal">
  <form class="modal-content animate" method="post" action="/KonkursMbg/edycja">
    ...
    <div class="container">
    <h2>Usuwanie edycji</h2>
    <h3>Czy na pewno chcesz usunąć tą edycję?</h3>
    <input type="hidden" id="id_edycja" name="id_edycja">
...
$(document).on('click', '.edit', function() {
    ...
    $('#m02 input[name=id_edycja]').val(rowData.eq(0).text());
    $('#m02 input[name=nazwa]').val(rowData.eq(1).text());
    $('#m02 input[name=url]').val(rowData.eq(2).text());
    $('#m02 select[name=active]').val(rowData.eq(3).text());
    $('#m02 select[name=wszystkieWyniki]').val(rowData.eq(4).text());
    document.getElementById('m02').style.display='block';
});

$(document).on('click', '.delete', function() {
    var rowData = $(this).closest('tr').find('td');
    $('#m03 input[name=id_edycja]').val(rowData.eq(0).text());
