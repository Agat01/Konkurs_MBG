...
<title>Uczestnicy konkursu wg województw </title>
...
String sql = "select nazwa from wojewodztwo order by nazwa";
...
%>
	<h2>Uczestnicy konkursu wg województw - <%= nazwaEdycji %></h2>
	<div id="selectorsDiv" class="mainDiv">
	<div style="width:50%;margin:0 auto;text-align:center;">
	<select id="wojewodztwo" onchange="">
		<option value="0"> </option>
		<option value="1">wszystkie województwa</option>
		<%
	        for(int i=0;i<wojewodztwa.size();i++) {
	        	wojewodztwoS=wojewodztwa.get(i).toString();
	        	int j=i+2;
	        %>
	        <option value="<%=j %>"><%=wojewodztwoS %></option>
	        <%} %>
...
document.getElementById("wojewodztwo").addEventListener("change", function() {
    ...
    xhr.open("GET", "uczestnicyWoj?wojewodztwo=" + selectedValue + "&edycja=<%= nazwaEdycji %>", true);
