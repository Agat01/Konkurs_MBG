...
String sql = "select id_wojewodztwo || ', ' || nazwa from wojewodztwo order by id_wojewodztwo";
Statement stmt = conn.createStatement();
ResultSet rs=stmt.executeQuery(sql);
while(rs.next()){
	wojewodztwo=rs.getString(1);
...
%>
<br>
<button id="dodaj" class="dodaj" onclick="document.getElementById('m01').style.display='block'" style="width:auto;">+ Dodaj nowy powiat</button>
<div id="m01" class="modal">
  <form class="modal-content animate" method="post" action="/KonkursMbg/powiat">
    <div class="imgcontainer">
      <span onclick="document.getElementById('m01').style.display='none'" class="close" title="Close Modal">&times;</span>
      
    </div>
    <div class="container">
    <h2>Dodawanie nowego powiatu</h2>
    <input type="hidden" id="metoda" name="metoda" value="add">
      <div>
        ...
        <div>
            ...
            <input type="text" list="wojewodztwa" id="wojewodztwo" name="wojewodztwo" onchange="get_data()" size="100"/>
			<datalist id="wojewodztwa">
						  <%
	        for(int i=0;i<wojewodztwa.size();i++) {
	        	wojewodztwoS=wojewodztwa.get(i).toString();
	        %>
	        <option value="<%=wojewodztwoS %>"><%=wojewodztwoS %></option>
	        <%} %>
			</datalist>
			<input type="hidden" id="woj" name="woj" value="">
        </div>
        <div>
...
<div id="m02" class="modal">
  <form class="modal-content animate" method="post" action="/KonkursMbg/powiat">
    ...
    <div class="container">
    <h2>Edytowanie powiatu</h2>
    <input type="hidden" id="id_powiat" name="id_powiat">
...
<div id="m03" class="modal">
  <form class="modal-content animate" method="post" action="/KonkursMbg/powiat">
    ...
    <div class="container">
    <h2>Usuwanie powiatu</h2>
    <h3>Czy na pewno chcesz usunąć ten powiat?</h3>
    <input type="hidden" id="id_powiat" name="id_powiat">
...
$(document).on('click', '.edit', function() {
    ...
    $('#m02 input[name=id_powiat]').val(rowData.eq(0).text());
    $('#m02 input[name=nazwa]').val(rowData.eq(1).text());
    $('#m02 input[name=wojewodztwo]').val(rowData.eq(2).text());
    document.getElementById('m02').style.display='block';
});

$(document).on('click', '.delete', function() {
    var rowData = $(this).closest('tr').find('td');
    $('#m03 input[name=id_powiat]').val(rowData.eq(0).text());
